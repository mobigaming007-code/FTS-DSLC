...
export default function viPhamPage() {
  useEffect(() => {
    ...
    function paginateRenderedList(key, container, requestedPage) {
      ...
      const items = Array.from(container.children).filter(
        (el) => el.dataset.adminPagination !== "1",
      );
      ...
      const currentPage = Math.min(
        Math.max(Number(requestedPage || __adminPagination[key]?.page || 1), 1),
        totalPages,
      );
      ...
      items.forEach((el, index) => {
        ...
        el.style.display =
          index >= start && index < start + ADMIN_PAGE_SIZE ? "" : "none";
      });
      ...
      const controls = pageNumbers
        .map((pageNumber) => {
          ...
          return (
            gap +
            '<button type="button" onclick="changeAdminPage(\'' +
            key +
            "', " +
            pageNumber +
            ')" class="min-w-8 rounded border px-2 py-1 text-xs font-bold ' +
            activeClass +
            '">' +
            pageNumber +
            "</button>"
      ...
      pager.className =
        "mt-4 flex flex-wrap items-center justify-center gap-1 border-t border-gray-100 pt-3";
    ...
    const __nativeFetch =
      (window as any).__adminNativeFetch || window.fetch.bind(window);
    ...
    function setGlobalProcessing(isLoading, message) {
      ...
      if (isLoading) {
        ...
        if (msg)
          msg.innerText =
            "Vui lòng không thực hiện thao tác nhiều lần. Hệ thống đang ghi nhận dữ liệu";
    ...
    function showBusyToast(message) {
      ...
      if (!toast) return;
      toast.innerText =
        message || "Hệ thống đang xử lý yêu cầu trước, vui lòng chờ.";
      toast.classList.add("active");
      clearTimeout(__busyToastTimer);
      __busyToastTimer = setTimeout(
    ...
    window.fetch = async function (resource, options = {}) {
      const url =
        typeof resource === "string"
          ...
          : resource && resource.url
            ? resource.url
            : "";
      const isAppsScriptApi =
        url === API_URL || url.includes("script.google.com/macros/s/");
      ...
      try {
        ...
        console.warn(
          "Không kết nối được API Apps Script: " +
        ...
        return new Response(
          JSON.stringify({
            success: false,
            error:
              "Không kết nối được API Apps Script. Kiểm tra mạng, quyền truy cập Apps Script hoặc deploy backend.",
          }),
          {
            status: 200,
    ...
    async function loadDashboard() {
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({ action: "getAdminDashboard", sessionId }),
        });
        ...
        if (res.success) {
          ...
          document.getElementById("stat-nolink").innerText =
            res.stats.noLinkGCN;
          ...
          if (res.recentLogs.length === 0)
            logsContainer.innerHTML =
              "<p class='italic text-gray-400'>Chưa có dữ liệu.</p>";
          else {
            logsContainer.innerHTML = res.recentLogs
              .map(
                (log) => `
    ...
    async function loadMinhChung(isSilent = false) {
      ...
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({
            action: "getDanhSachMinhChungChoXet",
            sessionId,
          }),
        });
        const res = await response.json();
        if (res.success) {
          ...
          if (res.data.length === 0) {
            container.innerHTML = "Đã duyệt hết MC.";
            return;
          }
    ...
    async function submitDuyetMC(maMC, status) {
      const soBuoi =
        parseFloat(document.getElementById("buoi-" + maMC).value) || 0;
      const ghiChu = document.getElementById("note-" + maMC).value;
      if (status === "Đã duyệt" && soBuoi <= 0)
        return alert("Nhập số buổi > 0");
      if (status === "Từ chối" && !ghiChu) return alert("Nhập lý do từ chối");

      const response = await fetch(API_URL, {
        method: "POST",
        body: JSON.stringify({
          action: "adminDuyetMinhChung",
          sessionId,
          maMC,
          status,
          soBuoi,
    ...
    async function loadGCN(isSilent = false) {
      ...
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({ action: "getDanhSachYeuCauGCN", sessionId }),
        });
        const res = await response.json();
        if (res.success) {
          ...
          if (isSilent) return;
          if (res.data.length === 0) {
            container.innerHTML = "Không có yêu cầu.";
            return;
          }
    ...
    async function loadNoLink(isSilent = false) {
      ...
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({ action: "getDanhSachNoLinkGCN", sessionId }),
        });
        const res = await response.json();
        if (res.success) {
          ...
          if (isSilent) return;
          if (res.data.length === 0) {
            container.innerHTML = "Không có hồ sơ nợ link.";
            return;
          }
    ...
    function getDriveThumbnailUrl(url) {
      ...
      return match
        ? "https://drive.google.com/thumbnail?id=" + match[0] + "&sz=w600"
        : value;
    }
    ...
    async function loadLichSuMC() {
      ...
      try {
        ...
        if (res.success) {
          if (res.data.length === 0) {
            container.innerHTML = "Chưa có lịch sử.";
            return;
          }
          ...
          container.innerHTML = res.data
            .map((item) => {
              ...
              const hangMuc = escapeAdminHtml(
                item.hangMuc || "Không có hạng mục",
              );
              const noiDungTNV = escapeAdminHtml(
                item.moTa || "Không có nội dung",
              );
    ...
    async function openEditMC(maMC, currentBuoi, currentNote) {
      ...
      const newStatus = prompt(
        "Nhập trạng thái mới (Đã duyệt / Từ chối):",
        "Đã duyệt",
      );
    ...
    async function loadLichSuGCN() {
      ...
      container.innerHTML =
        '<p class="text-center text-xs text-gray-500">Đang tải dữ liệu...</p>';
      ...
      try {
        ...
        if (res.success) {
          if (res.data.length === 0) {
            container.innerHTML =
              "<p class='text-center text-gray-400 text-xs'>Chưa có lịch sử GCN.</p>";
            return;
          }
        ...
        container.innerHTML =
          "<p class='text-center text-red-500 text-xs'>Lỗi kết nối hệ thống.</p>";
      }
    }
    ...
    async function openEditGCN(maYC, currentStatus, currentNote) {
      ...
      const newStatus = prompt(
        "Nhập trạng thái mới (Hoàn thành / Hẹn ngày trả / Từ chối):",
        currentStatus,
      );
      if (newStatus === null) return; // Người dùng nhấn Hủy

      const newNote = prompt(
        "Nhập lý do điều chỉnh hoặc ghi chú mới:",
        currentNote || "",
      );
      if (newNote === null) return;

      const urlGCN =
        newStatus === "Hoàn thành" ? prompt("Dán link GCN (nếu có):", "") : "";
    ...
    async function loadDiemTrucOptions() {
      ...
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({ action: "getDanhSachDiemTruc", sessionId }),
        });
        const res = await response.json();
        if (res.success) {
          sel.innerHTML =
            res.data
              .map(
                (d) =>
                  `<option value="${escapeText(d.value)}" data-is-ddb="${d.isDDB ? "1" : "0"}">${escapeText(d.label)}</option>`,
              )
              .join("") || '<option value="">Chưa có điểm</option>';
          autoSetHeSoTruongDiem();
        }
      } catch (e) {
        sel.innerHTML = '<option value="">Lỗi tải điểm</option>';
      }
    }
    ...
    function autoSetHeSoTruongDiem() {
      ...
      input.title = isDDB
        ? "Điểm đặc biệt DDB: có thể sửa hệ số theo thực tế"
        : "Điểm thường: mặc định hệ số 1";
    }
    ...
    async function traCuuTruongDiem() {
      ...
      document.getElementById("td-info").innerHTML =
        `<b>${res.data.hoTen}</b> (${res.data.maTNV})<br><span class="text-xs text-blue-600">SĐT: ${res.data.sdt || "N/A"} | Đã cộng: ${res.data.soBuoiCong} | Đã trừ: ${res.data.soBuoiTru} | Còn: ${res.data.tongBuoi}</span>`;
    }
    ...
    async function submitTruongDiem() {
      ...
      const heSoNhanRaw = (
        document.getElementById("td-heso")?.value || "1"
      ).replace(",", ".");
      const heSoNhan = Number(heSoNhanRaw) || 1;
      if (!ngayTruc || !buoi || !diemTruc)
        return alert("Chọn đủ ngày, buổi, điểm trực");
      if (heSoNhan <= 0) return alert("Hệ số phải lớn hơn 0");
      const response = await fetch(API_URL, {
        method: "POST",
        body: JSON.stringify({
          action: "adminThemTruongDiemTruc",
          sessionId,
          maTNV: selectedTruongDiemTNV.maTNV,
          ngayTruc,
          buoi,
          diemTruc,
          heSoNhan,
      ...
      const res = await response.json();
      if (res.success) {
        alert("Đã thêm trưởng điểm trực");
        loadTruongDiem();
    ...
    async function loadTruongDiem() {
      ...
      if (!box) return;
      box.innerHTML =
        '<p class="text-xs text-gray-400">Đang tải danh sách...</p>';
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({
            action: "getDanhSachTruongDiemTruc",
            sessionId,
          }),
        });
        const res = await response.json();
        if (res.success) {
          if (res.data.length === 0) {
            box.innerHTML =
              '<p class="text-xs text-gray-400 italic">Chưa có trưởng điểm trực.</p>';
            return;
          }
          ...
          box.innerHTML = `<p class="text-xs text-red-500">${res.error}</p>`;
      } catch (e) {
    ...
    async function deleteTruongDiem(rowNumber, maTNV) {
      ...
      if (
        !confirm(`Xóa phân công trưởng điểm của ${maTNV}?
    Thao tác này sẽ không công nhận quyền lợi điểm danh của Trưởng điểm trực.`)
      )
        return;
      const response = await fetch(API_URL, {
        method: "POST",
        body: JSON.stringify({
          action: "adminXoaTruongDiemTruc",
          sessionId,
          rowNumber,
          maTNV,
          adminName,
    ...
    async function submitViPham() {
      ...
      const linkMinhChung = document
        .getElementById("vp-minhchung")
        .value.trim();
      const quyetDinhXuPhat = document
        .getElementById("vp-quyetdinh")
        .value.trim();
      const linkQuyetDinhXuPhat = document
        .getElementById("vp-link")
        .value.trim();
      const soBuoiTru =
        parseFloat(document.getElementById("vp-tru").value) || 0;
    ...
    async function loadBaoCaoViPham() {
      ...
      if (!box) return;
      box.innerHTML =
        '<p class="text-xs text-gray-400">Đang tải báo cáo vi phạm...</p>';
      ...
      try {
        ...
        if (!res.data || res.data.length === 0) {
          box.innerHTML =
            '<p class="text-xs text-gray-400 italic">Chưa có báo cáo vi phạm.</p>';
          return;
        }
        ...
        box.innerHTML =
          '<p class="text-xs text-red-500">Lỗi tải báo cáo vi phạm.</p>';
      }
    }
    ...
    async function duyetBaoCaoViPham(maBaoCao) {
      ...
      const soBuoiTru =
        parseFloat(prompt("Nhập số buổi trừ chính thức:", "0")) || 0;
      if (soBuoiTru <= 0) return alert("Số buổi trừ phải > 0");

      const linkQuyetDinhXuPhat =
        prompt("Dán link quyết định/thông báo xử phạt nếu có:", "") || "";
    ...
    async function thucThiBaoCaoViPham(maBaoCao) {
      const ghiChuThucThi =
        prompt("Ghi chú thực thi/thông báo cho TNV:", "") || "";
      if (
        !confirm(
          "Xác nhận đã thực thi quyết định xử phạt? Thao tác này KHÔNG ghi thêm vào ViPham.",
        )
      )
        return;
    ...
    async function loadLichSuViPhamChinhThuc() {
      ...
      if (!box) return;
      box.innerHTML =
        '<p class="text-xs text-gray-400">Đang tải lịch sử vi phạm chính thức...</p>';
      ...
      try {
        ...
        if (res.success) {
          if (res.data.length === 0) {
            box.innerHTML =
              '<p class="text-xs text-gray-400 italic">Chưa có vi phạm chính thức.</p>';
            return;
          }
    ...
    async function deleteViPham(rowNumber, maTNV) {
      ...
      if (
        !confirm(
          `Xóa quyết định xử phạt chính thức của ${maTNV}?\nSố buổi trừ của bản ghi này sẽ không còn được tính.`,
        )
      )
        return;
    ...
    function selectAllKhuVucTaiKhoan() {
      ...
      Array.from(select.options).forEach(
        (opt) => (opt.selected = opt.value === "ALL"),
      );
    }
    ...
    async function initCapTaiKhoanVisibility() {
      ...
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({
            action: "getDanhSachTaiKhoanAdmin",
            sessionId,
          }),
        });
        const res = await response.json();
    ...
    async function loadKhuVucCapTaiKhoan() {
      ...
      try {
        const response = await fetch(API_URL, {
          method: "POST",
          body: JSON.stringify({
            action: "getDanhSachKhuVucCapTaiKhoan",
            sessionId,
          }),
        });
        const res = await response.json();
    ...
    async function submitCapTaiKhoan() {
      ...
      if (phamViKhuVuc.length === 0)
        return alert("Vui lòng chọn phạm vi khu vực.");
    ...
    async function loadDanhSachTaiKhoanAdmin() {
      ...
      if (!box) return;
      box.innerHTML =
        '<p class="text-xs text-gray-400">Đang tải danh sách tài khoản...</p>';
      ...
      try {
        ...
        box.innerHTML =
          '<p class="text-xs text-red-500">Lỗi tải danh sách tài khoản.</p>';
      }
    }
    ...
    function renderDanhSachTaiKhoanAdmin(data) {
      ...
      if (!data || data.length === 0) {
        box.innerHTML =
          '<p class="text-xs text-gray-400 italic">Chưa có tài khoản admin nào.</p>';
        return;
      }
    ...
    async function doiMatKhauTaiKhoan(email) {
      ...
      if (matKhauMoi.trim().length < 4)
        return alert("Mật khẩu mới nên có ít nhất 3 ký tự.");
    ...
    Object.assign(window as any, {
      changeAdminPage,
      getActionFromFetchOptions,
      setGlobalProcessing,
      setAdminButtonsDisabled,
      beginAdminProcessing,
      endAdminProcessing,
      showBusyToast,
      logout,
      switchTab,
      ...
      tuChoiBaoCaoViPham,
      thucThiBaoCaoViPham,
      loadLichSuViPhamChinhThuc,
      deleteViPham,
      getSelectedPhamViTaiKhoan,
      selectAllKhuVucTaiKhoan,
      onChangeCapQuyenTaiKhoan,
      initCapTaiKhoanVisibility,
      loadCapTaiKhoan,
      loadKhuVucCapTaiKhoan,
      submitCapTaiKhoan,
      loadDanhSachTaiKhoanAdmin,
      renderDanhSachTaiKhoanAdmin,
  ...
  return (
    <main className="min-h-screen bg-gray-100 pb-10">
      <nav className="bg-gray-900 text-white p-4 shadow-md sticky top-0 z-50 flex flex-wrap gap-3 items-center">
        <Link
          href="/admin/tong-quan"
          className="font-bold text-yellow-400 mr-auto"
        >
          ADMIN DSLC
        </Link>
        <span
        ...
        <button
          ...
          className="text-xs bg-red-600 hover:bg-red-700 px-3 py-1.5 rounded-lg font-bold"
        >
          Đăng xuất
        </button>
      </nav>
      <div className="max-w-5xl mx-auto px-2 pt-4 mb-4">
        <div className="menu-scroll bg-white p-1 rounded-xl shadow-sm border border-gray-200">
          <div className="flex space-x-1 min-w-max">
            <Link
              href="/admin/tong-quan"
              className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100"
            >
              Tổng quan
            </Link>
            <Link
              href="/admin/minh-chung"
              className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100"
            >
              Duyệt minh chứng
            </Link>
            ...
            <Link
              href="/admin/no-link-gcn"
              className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100"
            >
              Nợ link GCN
            </Link>
            <Link
              href="/admin/lich-su-minh-chung"
              className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100"
            >
              Lịch sử minh chứng
            </Link>
            <Link
              href="/admin/lich-su-gcn"
              className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100"
            >
              Lịch sử GCN
            </Link>
            <Link
              href="/admin/truong-diem"
              className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100"
            >
              Trưởng điểm
            </Link>
            <Link
              href="/admin/vi-pham"
              className="px-4 py-2 text-xs font-bold rounded-lg transition bg-gray-900 text-white"
            >
              Ghi nhận sai phạm
            </Link>
            <Link
              href="/admin/cap-tai-khoan"
              className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100"
            >
              Cấp tài khoản
            </Link>
      ...
      <section className="max-w-5xl mx-auto px-2">
        <div id="tab-vipham" className="tab-content active">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-red-50 px-4 py-3 border-b border-red-100 flex justify-between">
              <div>
                <h2 className="font-bold text-red-800 text-sm">
                  Ghi nhận / Duyệt sai phạm TNV
                </h2>
                <p className="text-[11px] text-red-600 mt-1">
                  Admin khu vực chỉ gửi báo cáo. Admin/SuperAdmin duyệt để trừ
                  buổi.
                </p>
              </div>
              <button
                onClick={() => {
                  (window as any).loadViPham();
                }}
            ...
            <div className="p-4 space-y-5 text-sm">
              <div className="bg-gray-50 border rounded-xl p-3">
                <h3 className="font-bold text-gray-800 text-xs uppercase mb-3">
                  1. Tra cứu TNV và ghi nhận sai phạm
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                  <input
                    id="vp-ma"
                    className="border p-2 rounded text-xs"
                    placeholder="Nhập mã TNV hoặc SĐT"
                  />
                  <button
                    onClick={() => {
                      (window as any).traCuuViPham();
                    }}
                    className="bg-gray-900 text-white font-bold rounded text-xs px-3 py-2"
                  >
                    Tra cứu
                  </button>
                  <input
                    id="vp-khuvuc"
                    className="border p-2 rounded text-xs md:col-span-2"
                    placeholder="Mã khu vực, ví dụ D1 / KV_MEDIA. Bỏ trống sẽ lấy phạm vi đầu tiên của admin khu vực"
                  />
                </div>
                <div
                  id="vp-info"
                  className="hidden bg-white border rounded-xl p-3 mt-3"
                ></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
                  ...
                  <input
                    id="vp-minhchung"
                    className="border p-2 rounded text-xs md:col-span-2"
                    placeholder="Link minh chứng báo cáo sai phạm (ảnh/video/Drive nếu có)"
                  />
                  <input
                    id="vp-quyetdinh"
                    className="border p-2 rounded text-xs"
                    placeholder="Quyết định xử phạt (Admin/SuperAdmin bắt buộc khi xử phạt trực tiếp)"
                  />
                  <input
                    id="vp-link"
                    className="border p-2 rounded text-xs"
                    placeholder="Link thông báo / quyết định"
                  />
                  <input
                    id="vp-tru"
                    type="number"
                    ...
                    min="0"
                    className="border p-2 rounded text-xs"
                    placeholder="Số buổi trừ / đề xuất trừ"
                  />
                  <button
                    onClick={() => {
                      (window as any).submitViPham();
                    }}
                    className="bg-red-700 text-white font-bold rounded text-xs px-3 py-2"
                  >
                    Gửi ghi nhận / báo cáo sai phạm
                  </button>
                </div>
                <p className="text-[11px] text-gray-500 mt-2 leading-relaxed">
                  Nếu là Admin khu vực: hệ thống chỉ tạo báo cáo chờ duyệt. Nếu
                  là Admin/SuperAdmin: hệ thống tự động ghi quyết định thực thi
                  ngay lập tức.
                </p>
              </div>
              ...
              <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-3">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="font-bold text-yellow-800 text-xs uppercase">
                    2. Báo cáo sai phạm / Chờ duyệt / Thực thi
                  </h3>
                  <button
                    onClick={() => {
                      (window as any).loadBaoCaoViPham();
                    }}
                    className="text-xs text-blue-600 underline"
                  >
                    Tải báo cáo
              ...
              <div className="bg-white border rounded-xl p-3">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="font-bold text-gray-800 text-xs uppercase">
                    3. Vi phạm chính thức đang tính trừ buổi
                  </h3>
                  <button
                    onClick={() => {
                      (window as any).loadLichSuViPhamChinhThuc();
                    }}
                    className="text-xs text-blue-600 underline"
                  >
                    Tải lịch sử chính thức
                  </button>
                </div>
